// ==UserScript==
// @name         提示词助手 Pro v13.1 (多平台支持版)
// @namespace    http://tampermonkey.net/
// @version      13.1
// @description  新增豆包、ChatGPT、即梦平台支持，优化分类拖拽排序
// @author       DeepSeekUser
...
(function() {
    ...
    // ==========================================
    const DB_KEY = 'ds_helper_v13_storage';
    const DB_VERSION = '13.1';
    ...
    const Store = {
            ...
            try {
                ...
                if (!parsed.version || parsed.version !== DB_VERSION) {
                    ...
                    if (!parsed.categoryOrder) {
                        parsed.categoryOrder = parsed.categories || [];
                    }
        ...
        load(force = false) {
            ...
            try {
                ...
                const result = JSON.parse(data || '{"version":"' + DB_VERSION + '","prompts":[],"categories":[],"categoryOrder":[]}');
                
                // 兼容旧数据
                if (!result.categoryOrder && result.categories) {
                    result.categoryOrder = [...result.categories];
                }
                ...
                return { version: DB_VERSION, prompts: [], categories: [], categoryOrder: [] };
            }
        },
        ...
        save(data) {
            ...
            if (data.categories && data.categoryOrder) {
                // 移除categoryOrder中不存在的分类
                data.categoryOrder = data.categoryOrder.filter(cat => 
                    data.categories.includes(cat)
                );
                
                // 添加categories中存在但categoryOrder中不存在的分类
                data.categories.forEach(cat => {
                    if (!data.categoryOrder.includes(cat)) {
                        data.categoryOrder.push(cat);
                    }
                });
            } else if (data.categories && !data.categoryOrder) {
                data.categoryOrder = [...data.categories];
            }
        ...
        resetFactory() {
            ...
            const data = {
                ...
                categoryOrder: [...new Set(categories)],
        ...
        upsertPrompt(obj) {
            ...
            if (obj.category && !data.categories.includes(obj.category)) {
                ...
                if (!data.categoryOrder.includes(obj.category)) {
                    data.categoryOrder.push(obj.category);
                }
            }
        ...
        renameCategory(oldName, newName) {
            ...
            const orderIdx = data.categoryOrder.indexOf(oldName);
            
            if (idx !== -1) {
                data.categories[idx] = newName;
                
                if (orderIdx !== -1) {
                    data.categoryOrder[orderIdx] = newName;
                }
        ...
        deleteCategory(catName) {
            ...
            const orderIdx = data.categoryOrder.indexOf(catName);
            
            if (idx !== -1) {
                data.categories.splice(idx, 1);
                
                if (orderIdx !== -1) {
                    data.categoryOrder.splice(orderIdx, 1);
                }
        ...
        reorderCategories(newOrder) {
            ...
            if (allCategories.size !== orderSet.size || 
                ![...allCategories].every(cat => orderSet.has(cat))) {
                console.error('分类排序无效：分类数量不匹配');
                return false;
            }
            
            data.categoryOrder = newOrder;
            this.save(data);
            return true;
        },
        ...
        moveCategory(catName, direction) {
            const data = this.load();
            const order = data.categoryOrder || [...data.categories];
            const index = order.indexOf(catName);
        ...
        importPrompts(promptsArray, fallbackCategory = "") {
            ...
            promptsArray.forEach(prompt => {
                ...
                if (category && !data.categories.includes(category)) {
                    ...
                    if (!data.categoryOrder.includes(category)) {
    ...
    const styles = `
        :root {
            ...
            --v12-drag-color: rgba(77, 107, 254, 0.2);
        }
        .dark :root {
            ...
            --v12-drag-color: rgba(77, 107, 254, 0.3);
        }
        ...
        .v12-btn-warning:hover {
            background: var(--v12-warning);
            color: #fff;
            border-color: var(--v12-warning);
        }
        ...
        .v12-cat-group-actions {
            display: flex;
            gap: 4px;
            opacity: 0.6;
        }
        
        .v12-cat-group-action-btn {
            width: 20px;
            height: 20px;
            border-radius: 4px;
            display: flex;
        ...
        .v12-cat-group-action-btn:hover {
        ...
        .v12-item-drag-handle {
            cursor: move;
            opacity: 0.3;
            margin-right: 8px;
            transition: opacity var(--v12-animation-speed) ease;
            display: flex;
            align-items: center;
            justify-content: center;
            width: 16px;
        ...
        .v12-item:hover .v12-item-drag-handle {
            opacity: 0.6;
        }
        ...
        .v12-item:hover .v12-item-actions {
            opacity: 1;
        }
        ...
        .v12-item-action-btn:hover {
            background: rgba(127,127,127,0.15);
        }
        
        .v12-item-action-edit:hover {
            background: rgba(77,107,254,0.15);
            color: var(--v12-primary);
        }
        
        .v12-item-action-delete:hover {
        ...
        .v12-item.dragging {
            opacity: 0.5;
            background: var(--v12-drag-color);
            border: 2px dashed var(--v12-primary);
        }
        
        .v12-item.drag-over {
            border-top: 2px solid var(--v12-primary);
        }
        ...
        .v12-cat-manage-section {
            margin-top: 20px;
            padding-top: 20px;
            border-top: 1px solid var(--v12-border);
        }
        
        .v12-cat-manage-title {
            font-size: 12px;
            color: var(--v12-text);
            opacity: 0.4;
            margin-bottom: 12px;
        ...
        .v12-cat-manage-btn:hover {
            background: rgba(77,107,254,0.1);
            border-color: var(--v12-primary);
            color: var(--v12-primary);
        }
        
        .v12-cat-manage-btn-danger:hover {
            background: rgba(239,68,68,0.1);
            border-color: var(--v12-danger);
            color: var(--v12-danger);
        }
        
        .v12-cat-manage-btn-warning:hover {
            background: rgba(245,158,11,0.1);
            border-color: var(--v12-warning);
            color: var(--v12-warning);
        }
        
        .v12-drag-instruction {
            font-size: 11px;
            color: var(--v12-text);
            opacity: 0.4;
            margin-top: 10px;
    ...
    function detectPlatform() {
        ...
        if (hostname.includes('doubao.com')) {
            ...
            return 'chatgpt';
        } else if (hostname.includes('jimeng.jianying.com')) {
            return 'jimeng';
    ...
    function injectToolbarButton() {
        ...
        console.log(`尝试注入提示词助手按钮到 ${currentPlatform}...`);
        ...
        // 获取当前平台的选择器
        const platformSpecificSelectors = platformSelectors[currentPlatform] || platformSelectors.deepseek;
        ...
        if (bestContainer) {
            ...
            button.title = '提示词助手 Pro v' + DB_VERSION;
            button.innerHTML = `
            ...
            try {
                ...
                console.log(`✅ 提示词助手按钮注入成功到 ${currentPlatform}`);
    ...
    function createFloatButton() {
        ...
        console.log(`✅ 浮动按钮创建成功（${currentPlatform}工具栏按钮注入失败）`);
    }
    ...
    function createBaseDOM() {
        ...
        wrap.innerHTML = `
            ...
            <div id="v12-main-overlay" class="v12-overlay v12-clickable">
                <div id="v12-panel">
                    <div class="v12-header">
                        <div class="v12-title">提示词助手 Pro <span style="font-size:12px; opacity:0.4; font-weight:400;">v${DB_VERSION}</span></div>
                    ...
                    <div class="v12-body">
                        <div class="v12-sidebar" id="v12-sb">
                            <div id="v12-sidebar-content"></div>
                            <div id="v12-cat-management" class="v12-cat-manage-section">
                                <div class="v12-cat-manage-title">📁 分类管理</div>
                                <div class="v12-cat-manage-buttons">
                                    <button class="v12-cat-manage-btn" id="v12-cat-add-new">
                                        <span>➕</span> 新建分类
                                    </button>
                                    <button class="v12-cat-manage-btn v12-cat-manage-btn-warning" id="v12-cat-reorder">
                                        <span>↕️</span> 重置排序
                                    </button>
                                    <button class="v12-cat-manage-btn v12-cat-manage-btn-danger" id="v12-cat-cleanup">
                                        <span>🗑️</span> 清理空分类
                                    </button>
                                </div>
                                <div class="v12-drag-instruction">提示：拖动左侧把手可以重新排序</div>
                            </div>
                        </div>
            ...
            <!-- 新建分类模态框 -->
            <div id="v12-newcat-overlay" class="v12-overlay" style="z-index:10005; display:none;">
                <div class="v12-modal" style="width:420px; padding:30px;">
                    <h3 style="margin:0;">新建分类</h3>
                    <input type="text" class="v12-input" id="v12-newcat-val" placeholder="请输入分类名称">
                    <div style="display:flex; justify-content:flex-end; gap:12px;">
                        <button class="v12-btn" id="v12-newcat-cancel">取消</button>
                        <button class="v12-btn" id="v12-newcat-save" style="background:var(--v12-success); color:#fff; border:none;">创建分类</button>
                    </div>
                </div>
            </div>
        ...
        cachedElements = {
            ...
            'v12-sidebar-content': wrap.querySelector('#v12-sidebar-content'),
    ...
    function renderAll() {
        ...
        renderQueue = requestAnimationFrame(() => {
            ...
            const sidebarContent = cachedElements['v12-sidebar-content'];
            const grid = cachedElements['v12-grid'];
            const srchVal = cachedElements['v12-srch'].value.toLowerCase().trim();
            
            // 使用categoryOrder来排序分类
            const orderedCategories = data.categoryOrder || data.categories;
            ...
            header.className = 'v12-cat-group';
            header.innerHTML = `
                <span>🗂️ 所有分类 (${orderedCategories.length})</span>
                <div class="v12-cat-group-actions">
                    <button class="v12-cat-group-action-btn" id="v12-cat-collapse-all" title="折叠所有">−</button>
                    <button class="v12-cat-group-action-btn" id="v12-cat-expand-all" title="展开所有">＋</button>
                </div>
            `;
            sbFragment.appendChild(header);
            ...
            orderedCategories.forEach(cat => {
                ...
                item.innerHTML = `
                    <div class="v12-item-drag-handle" title="拖动排序">⋮⋮</div>
                    <div class="v12-item-content">
                        <div class="v12-item-title">${cat}</div>
                        <span class="v12-item-count">${count}</span>
                    </div>
                    <div class="v12-item-actions">
                        <button class="v12-item-action-btn v12-item-action-edit" title="重命名">✎</button>
                        <button class="v12-item-action-btn v12-item-action-delete" title="删除分类">🗑</button>
                    </div>
                `;
                
                // 点击选择分类
                item.onclick = (e) => {
                    // 如果点击的是操作按钮或拖动把手，不触发分类选择
                    if (e.target.closest('.v12-item-actions') || e.target.closest('.v12-item-drag-handle')) {
                        return;
                    }
                ...
                // 操作按钮事件
                const actionButtons = item.querySelectorAll('.v12-item-action-btn');
                actionButtons.forEach(btn => {
                    btn.onclick = (e) => {
                        e.stopPropagation();
                        
                        if (btn.classList.contains('v12-item-action-edit')) {
                            openRename(cat);
                        } else if (btn.classList.contains('v12-item-action-delete')) {
                            deleteCat(cat);
                        }
                    };
                });
            ...
            sidebarContent.appendChild(sbFragment);
    ...
    function handleDrop(e) {
        ...
        if (dragSource && dragSource !== this) {
            const sourceCat = dragSource.getAttribute('data-category');
            ...
            if (sourceCat && targetCat && sourceCat !== targetCat) {
                ...
                const data = Store.load();
                const order = data.categoryOrder || data.categories;
                const sourceIndex = order.indexOf(sourceCat);
                const targetIndex = order.indexOf(targetCat);
                
                if (sourceIndex !== -1 && targetIndex !== -1) {
                    // 从原位置移除
                    order.splice(sourceIndex, 1);
                    // 插入到新位置
                    order.splice(targetIndex, 0, sourceCat);
                    
                    Store.reorderCategories(order);
                    renderAll();
                    showToast(`分类 "${sourceCat}" 已移动到新位置`);
                }
            }
        }
    ...
    function handleDragEnd(e) {
        document.querySelectorAll('.v12-item').forEach(item => {
    ...
    function insertToChat(text) {
        ...
        switch(currentPlatform) {
            case 'deepseek':
                ...
                break;
            case 'doubao':
                area = document.querySelector('textarea, [contenteditable="true"], [role="textbox"]');
                break;
            case 'chatgpt':
                area = document.querySelector('#prompt-textarea, textarea, [contenteditable="true"], [role="textbox"]');
                break;
            case 'jimeng':
                area = document.querySelector('textarea, [contenteditable="true"], [role="textbox"]');
                break;
        ...
        if (!area) {
            // 备用方案：查找所有可能的输入框
            const possibleAreas = document.querySelectorAll('textarea, [contenteditable="true"], [role="textbox"], [data-testid="send-button"] + div');
            for (const elem of possibleAreas) {
        ...
        try {
            // 判断是否是contenteditable元素
            if (area.hasAttribute('contenteditable') || area.getAttribute('role') === 'textbox') {
                // 处理contenteditable元素
                const currentHTML = area.innerHTML || '';
                const newHTML = currentHTML.trim() ? currentHTML + '<br><br>' + text.replace(/\n/g, '<br>') : text.replace(/\n/g, '<br>');
    ...
    function openNewCategory() {
        getElement('v12-newcat-val').value = '';
        getElement('v12-newcat-overlay').style.display = 'block';
        setTimeout(() => getElement('v12-newcat-val').focus(), 100);
    }
    ...
    function cleanupEmptyCategories() {
        ...
        const emptyCategories = data.categories.filter(cat => 
            cat !== DEFAULT_CAT && data.prompts.filter(p => p.category === cat).length === 0
        );
        ...
        if (confirm(`找到 ${emptyCategories.length} 个空分类，是否删除？\n\n空分类列表：\n${emptyCategories.join('\n')}`)) {
            let deletedCount = 0;
            emptyCategories.forEach(cat => {
                if (Store.deleteCategory(cat)) {
                    deletedCount++;
                }
            });
            ...
            if (deletedCount > 0) {
                renderAll();
                showToast(`已删除 ${deletedCount} 个空分类`);
            }
        }
    }
    ...
    function resetCategoryOrder() {
        ...
        const sorted = [...data.categories].sort((a, b) => {
            if (a === DEFAULT_CAT) return -1;
            if (b === DEFAULT_CAT) return 1;
            return a.localeCompare(b);
        });
        
        if (confirm('确定要重置分类排序吗？分类将按字母顺序重新排列。')) {
            Store.reorderCategories(sorted);
            renderAll();
            showToast('分类排序已重置');
        }
    }
    ...
    function bindGlobalEvents() {
        ...
        // 分类管理按钮
        getElement('v12-cat-add-new').onclick = openNewCategory;
        getElement('v12-cat-cleanup').onclick = cleanupEmptyCategories;
        getElement('v12-cat-reorder').onclick = resetCategoryOrder;
        
        // 新建分类模态框事件
        getElement('v12-newcat-save').onclick = () => {
            const newCatName = getElement('v12-newcat-val').value.trim();
            if (!newCatName) {
                alert('请输入分类名称');
                return;
            }
            ...
            // 添加新分类
            data.categories.push(newCatName);
            data.categoryOrder.push(newCatName);
            Store.save(data);
            
            getElement('v12-newcat-overlay').style.display = 'none';
            currentActiveCat = newCatName;
            renderAll();
            showToast(`新分类 "${newCatName}" 已创建`);
        };
        
        getElement('v12-newcat-cancel').onclick = () => {
            getElement('v12-newcat-overlay').style.display = 'none';
        };
        ...
        getElement('v12-exp-json').onclick = () => {
            ...
            downloadFile(
                ...
                `提示词库_${new Date().toISOString().split('T')[0]}.json`, 
                'application/json'
            );
        ...
        getElement('v12-exp-csv').onclick = () => {
            ...
            downloadFile(
                csv,
                `提示词库_${new Date().toISOString().split('T')[0]}.csv`,
        ...
        getElement('v12-exp-txt').onclick = () => {
            ...
            downloadFile(
                txt,
                `提示词库_${new Date().toISOString().split('T')[0]}.txt`,
